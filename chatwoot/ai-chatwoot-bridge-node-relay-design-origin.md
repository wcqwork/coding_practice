...
如启用第二次 AI 重试，可增加 `cw.ai.retry.120s`。默认 `AI_MAX_ATTEMPTS=1` 时，AI 失败直接转人工。
...
flowchart TD
  ...
  HIST --> AI[调用 Dify / FastGPT，带历史]
  AI -- 可重试失败 --> RETRY[确认发布 retry queue 后 ack 原消息]
  AI -- 最终失败 --> HO
...
AI 一次约 2 分钟，处理时间越长越容易在中间出问题，所以重复调 AI 不是小概率。这是本方案明确接受的代价。
...
- 主动转人工数量；
- **AI 调用次数 vs 客户消息数**（比值 > 1 说明重复消费在烧额度）。
...
| 4 | retry queue + confirm-before-ack | 重试发布失败时原消息不丢 |
...
- **重复 delivery 会重复调用 AI**，监控 `redelivered` 指标和"AI 调用次数/客户消息数"比值，确认额度浪费在可接受范围。
...
Worker：拉历史、AI 处理、手动 ack、重试转发、source_id 幂等回复、失败转人工
Dify/FastGPT：生成答案
```
...
> Chatwoot webhook 在几秒内结束；RabbitMQ confirm 后才接管责任；AI 两分钟后通过官方消息 API 回写；Worker 只有在业务结果或后继消息已经可靠保存后才 ack。重复消费会重复调用 AI（接受），但 `source_id` 保证不重复回复。单节点 RabbitMQ 的主机/磁盘故障仍属于风险边界，需要 3 节点 quorum queue 才能提供 Broker 高可用。
